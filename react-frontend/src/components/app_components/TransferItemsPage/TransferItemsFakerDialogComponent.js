...
import transferItemsFakerFactory from "./transferItemsFakerFactory";
...
const TransferItemsFakerDialogComponent = (props) => {
...
const [partIds, setpartIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("transferDetails")
...
client.service("partsMaster")
    ...
    const onRun = async () => {
        let fakeData = transferItemsFakerFactory(props.user,fakerCount,transferDateIds,partIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("transferItems").create(elem));
...
export default connect(mapState, mapDispatch)(TransferItemsFakerDialogComponent);
