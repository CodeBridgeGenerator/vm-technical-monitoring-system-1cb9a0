...
const TransferItemsCreateDialogComponent = (props) => {
...
const [part, setPart] = useState([])
    ...
    const onSave = async () => {
        ...
        const result = await client.service("transferItems").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Transfer Items updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Transfer Items" });
        }
        setLoading(false);
    };
    ...
    useEffect(() => {
                    // on mount transferDetails
                    client
                        .service("transferDetails")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleTransferDetailsId } })
                        .then((res) => {
...
const partOptions = part.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Transfer Items" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="transferItems-create-dialog-component">
...
export default connect(mapState, mapDispatch)(TransferItemsCreateDialogComponent);
