...
import machineMasterRawFakerFactory from "./machineMasterRawFakerFactory";
...
const MachineMasterRawFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = machineMasterRawFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("machineMasterRaw").create(elem));
...
export default connect(mapState, mapDispatch)(MachineMasterRawFakerDialogComponent);
