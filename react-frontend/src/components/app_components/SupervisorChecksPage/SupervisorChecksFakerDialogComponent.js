...
import supervisorChecksFakerFactory from "./supervisorChecksFakerFactory";
...
const SupervisorChecksFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [supervisorCheckListIdIds, setsupervisorCheckListIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("externalChecklists")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setsupervisorCheckListIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = supervisorChecksFakerFactory(props.user,fakerCount,supervisorCheckListIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("supervisorChecks").create(elem));
...
export default connect(mapState, mapDispatch)(SupervisorChecksFakerDialogComponent);
