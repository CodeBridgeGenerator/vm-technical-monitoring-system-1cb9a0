...
import AtlasTicketsDatatable from "./AtlasTicketsDataTable";
import AtlasTicketsEditDialogComponent from "./AtlasTicketsEditDialogComponent";
import AtlasTicketsCreateDialogComponent from "./AtlasTicketsCreateDialogComponent";
import AtlasTicketsFakerDialogComponent from "./AtlasTicketsFakerDialogComponent";
import AtlasTicketsSeederDialogComponent from "./AtlasTicketsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const AtlasTicketsPage = (props) => {
    ...
    const filename = "atlasTickets.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("atlasTickets");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Atlas Tickets", type: "error", message: error.message || "Failed get Atlas Tickets" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("atlasTickets").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Atlas Tickets", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("atlasTickets").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/atlasTickets/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="atlasTickets-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="atlasTickets-datatable">
                    <AtlasTicketsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <AtlasTicketsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AtlasTicketsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <AtlasTicketsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AtlasTicketsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AtlasTicketsPage);
