...
import atlasTicketsFakerFactory from "./atlasTicketsFakerFactory";
...
const AtlasTicketsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = atlasTicketsFakerFactory(props.user,fakerCount,machineIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("atlasTickets").create(elem));
...
export default connect(mapState, mapDispatch)(AtlasTicketsFakerDialogComponent);
