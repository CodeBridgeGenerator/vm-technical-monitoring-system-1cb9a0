...
import IrmsDeliveryOrdersDatatable from "./IrmsDeliveryOrdersDataTable";
...
import IrmsDeliveryOrdersFakerDialogComponent from "./IrmsDeliveryOrdersFakerDialogComponent";
import IrmsDeliveryOrdersSeederDialogComponent from "./IrmsDeliveryOrdersSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IrmsDeliveryOrdersPage = (props) => {
    ...
    const filename = "irmsDeliveryOrders.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("irmsDeliveryOrders");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , purchaseOrder : urlParams.singleCustomerPurchaseOrdersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "purchaseOrder",
                        service : "customer_purchase_orders",
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Delivery Orders", type: "error", message: error.message || "Failed get Delivery Orders" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("irmsDeliveryOrders").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Delivery Orders", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("irmsDeliveryOrders").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/irmsDeliveryOrders/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="irmsDeliveryOrders-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="irmsDeliveryOrders-datatable">
                    <IrmsDeliveryOrdersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IrmsDeliveryOrdersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IrmsDeliveryOrdersCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IrmsDeliveryOrdersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IrmsDeliveryOrdersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IrmsDeliveryOrdersPage);
