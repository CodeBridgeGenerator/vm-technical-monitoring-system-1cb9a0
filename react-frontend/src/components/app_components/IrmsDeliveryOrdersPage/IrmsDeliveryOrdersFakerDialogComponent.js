...
import irmsDeliveryOrdersFakerFactory from "./irmsDeliveryOrdersFakerFactory";
...
const IrmsDeliveryOrdersFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("customerPurchaseOrders")
                        .find({ query: { $select: ["_id","purchaseOrderId"] } })
                        .then((data) => {
                        setpurchaseOrderIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = irmsDeliveryOrdersFakerFactory(props.user,fakerCount,purchaseOrderIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("irmsDeliveryOrders").create(elem));
...
export default connect(mapState, mapDispatch)(IrmsDeliveryOrdersFakerDialogComponent);
