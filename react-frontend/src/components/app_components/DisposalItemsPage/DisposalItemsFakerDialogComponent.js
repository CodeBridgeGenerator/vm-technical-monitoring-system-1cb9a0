...
import disposalItemsFakerFactory from "./disposalItemsFakerFactory";
...
const DisposalItemsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [associatedNoIds, setassociatedNoIds] = useState();
const [partIds, setpartIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("disposalDetails")
                        .find({ query: { $select: ["_id","associatedNumber"] } })
                        .then((data) => {
                        setassociatedNoIds(data.data);
                        });
client.service("partsMaster")
    ...
    const onRun = async () => {
        let fakeData = disposalItemsFakerFactory(props.user,fakerCount,associatedNoIds,partIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("disposalItems").create(elem));
...
export default connect(mapState, mapDispatch)(DisposalItemsFakerDialogComponent);
