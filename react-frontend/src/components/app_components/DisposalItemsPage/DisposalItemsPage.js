...
import DisposalItemsDatatable from "./DisposalItemsDataTable";
import DisposalItemsEditDialogComponent from "./DisposalItemsEditDialogComponent";
import DisposalItemsCreateDialogComponent from "./DisposalItemsCreateDialogComponent";
import DisposalItemsFakerDialogComponent from "./DisposalItemsFakerDialogComponent";
import DisposalItemsSeederDialogComponent from "./DisposalItemsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const DisposalItemsPage = (props) => {
    ...
    const filename = "disposalItems.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("disposalItems");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , associatedNo : urlParams.singleDisposalDetailsId ,part : urlParams.singlePartsMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Disposal Items", type: "error", message: error.message || "Failed get Disposal Items" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("disposalItems").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Disposal Items", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("disposalItems").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/disposalItems/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="disposalItems-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="disposalItems-datatable">
                    <DisposalItemsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <DisposalItemsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DisposalItemsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <DisposalItemsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DisposalItemsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(DisposalItemsPage);
