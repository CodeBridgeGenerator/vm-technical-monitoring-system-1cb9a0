...
const ExternalMachinesEditDialogComponent = (props) => {
...
commissionDate: _entity?.commissionDate,
        ...
        await client.service("externalMachines").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info externalMachines updated successfully" });
...
const vendingMachineTypeOptions = vendingMachineType.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit External Machines" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="externalMachines-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(ExternalMachinesEditDialogComponent);
