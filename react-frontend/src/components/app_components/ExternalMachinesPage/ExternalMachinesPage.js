...
import ExternalMachinesDatatable from "./ExternalMachinesDataTable";
import ExternalMachinesEditDialogComponent from "./ExternalMachinesEditDialogComponent";
import ExternalMachinesCreateDialogComponent from "./ExternalMachinesCreateDialogComponent";
import ExternalMachinesFakerDialogComponent from "./ExternalMachinesFakerDialogComponent";
import ExternalMachinesSeederDialogComponent from "./ExternalMachinesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ExternalMachinesPage = (props) => {
    ...
    const filename = "externalMachines.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("externalMachines");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("externalMachines")
            .find({ query: { $limit: 10000 , ownership : urlParams.singleBranchesId ,vendingMachineType : urlParams.singleVendingMachinesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "External Machines", type: "error", message: error.message || "Failed get External Machines" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("externalMachines").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "External Machines", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("externalMachines").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/externalMachines/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>External Machines </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="externalMachines-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="externalMachines-datatable">
                    <ExternalMachinesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ExternalMachinesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ExternalMachinesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ExternalMachinesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ExternalMachinesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ExternalMachinesPage);
