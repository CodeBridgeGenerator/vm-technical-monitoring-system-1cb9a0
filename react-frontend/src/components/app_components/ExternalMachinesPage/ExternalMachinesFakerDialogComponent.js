...
import externalMachinesFakerFactory from "./externalMachinesFakerFactory";
...
const ExternalMachinesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [ownershipIds, setownershipIds] = useState();
const [vendingMachineTypeIds, setvendingMachineTypeIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("branches")
                        ...
                        .then((data) => {
                        setownershipIds(data.data);
                        });
client.service("vendingMachines")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setvendingMachineTypeIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = externalMachinesFakerFactory(props.user,fakerCount,ownershipIds,vendingMachineTypeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("externalMachines").create(elem));
...
export default connect(mapState, mapDispatch)(ExternalMachinesFakerDialogComponent);
