...
const ExternalMachinesCreateDialogComponent = (props) => {
...
const [vendingMachineType, setVendingMachineType] = useState([])
    ...
    const onSave = async () => {
        ...
        const result = await client.service("externalMachines").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info External Machines updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in External Machines" });
        }
        setLoading(false);
    };
...
const vendingMachineTypeOptions = vendingMachineType.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create External Machines" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="externalMachines-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="commissionDate"  value={_entity?.commissionDate ? new Date(_entity?.commissionDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("commissionDate", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(ExternalMachinesCreateDialogComponent);
