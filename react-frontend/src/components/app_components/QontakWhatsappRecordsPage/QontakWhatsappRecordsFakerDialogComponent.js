...
import qontakWhatsappRecordsFakerFactory from "./qontakWhatsappRecordsFakerFactory";
...
const QontakWhatsappRecordsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = qontakWhatsappRecordsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("qontakWhatsappRecords").create(elem));
...
export default connect(mapState, mapDispatch)(QontakWhatsappRecordsFakerDialogComponent);
