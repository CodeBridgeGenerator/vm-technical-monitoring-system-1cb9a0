...
import QontakWhatsappRecordsDatatable from "./QontakWhatsappRecordsDataTable";
import QontakWhatsappRecordsEditDialogComponent from "./QontakWhatsappRecordsEditDialogComponent";
import QontakWhatsappRecordsCreateDialogComponent from "./QontakWhatsappRecordsCreateDialogComponent";
import QontakWhatsappRecordsFakerDialogComponent from "./QontakWhatsappRecordsFakerDialogComponent";
import QontakWhatsappRecordsSeederDialogComponent from "./QontakWhatsappRecordsSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const QontakWhatsappRecordsPage = (props) => {
    ...
    const filename = "qontakWhatsappRecords";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("qontakWhatsappRecords");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Qontak Whatsapp Records", type: "error", message: error.message || "Failed get Qontak Whatsapp Records" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("qontakWhatsappRecords").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Qontak Whatsapp Records", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("qontakWhatsappRecords").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Qontak Whatsapp Records",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/qontakWhatsappRecords/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Qontak Whatsapp Records </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="qontakWhatsappRecords-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="qontakWhatsappRecords-datatable">
                    <QontakWhatsappRecordsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <QontakWhatsappRecordsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <QontakWhatsappRecordsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <QontakWhatsappRecordsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <QontakWhatsappRecordsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(QontakWhatsappRecordsPage);
