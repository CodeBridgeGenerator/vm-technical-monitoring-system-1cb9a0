...
const QontakWhatsappRecordsEditDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            channelRoomId: _entity?.channelRoomId,
qrText: _entity?.qrText,
extractedMachineId: _entity?.extractedMachineId,
vmCode: _entity?.vmCode,
accountUniqueId: _entity?.accountUniqueId,
customerPhoneNo: _entity?.customerPhoneNo,
refNo: _entity?.refNo,
status: _entity?.status,
landingUrl: _entity?.landingUrl,
source: _entity?.source,
        ...
        const result = await client.service("qontakWhatsappRecords").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info qontakWhatsappRecords updated successfully" });
    ...
    return (
        <Dialog header="Edit Qontak Whatsapp Records" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="qontakWhatsappRecords-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(QontakWhatsappRecordsEditDialogComponent);
