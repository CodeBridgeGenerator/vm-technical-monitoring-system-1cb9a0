...
import vendingMachinesFakerFactory from "./vendingMachinesFakerFactory";
...
const VendingMachinesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = vendingMachinesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vendingMachines").create(elem));
...
export default connect(mapState, mapDispatch)(VendingMachinesFakerDialogComponent);
