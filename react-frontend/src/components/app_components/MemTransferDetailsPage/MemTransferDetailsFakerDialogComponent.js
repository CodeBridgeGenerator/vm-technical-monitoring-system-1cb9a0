...
import memTransferDetailsFakerFactory from "./memTransferDetailsFakerFactory";
...
const MemTransferDetailsFakerDialogComponent = (props) => {
...
client.service("memWarehouses")
    ...
    const onRun = async () => {
        let fakeData = memTransferDetailsFakerFactory(props.user,fakerCount,sourceWarehouseIds,destinationWarehouseIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memTransferDetails").create(elem));
...
export default connect(mapState, mapDispatch)(MemTransferDetailsFakerDialogComponent);
