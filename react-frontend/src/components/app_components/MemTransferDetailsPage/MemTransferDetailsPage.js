...
import MemTransferDetailsDatatable from "./MemTransferDetailsDataTable";
...
import MemTransferDetailsFakerDialogComponent from "./MemTransferDetailsFakerDialogComponent";
import MemTransferDetailsSeederDialogComponent from "./MemTransferDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MemTransferDetailsPage = (props) => {
    ...
    const filename = "memTransferDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("memTransferDetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , sourceWarehouse : urlParams.singleMemWarehousesId ,destinationWarehouse : urlParams.singleMemWarehousesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Mem Transfer Details", type: "error", message: error.message || "Failed get Mem Transfer Details" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("memTransferDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Mem Transfer Details", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("memTransferDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Mem Transfer Details",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/memTransferDetails/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Mem Transfer Details </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="memTransferDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="memTransferDetails-datatable">
                    <MemTransferDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MemTransferDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MemTransferDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MemTransferDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MemTransferDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MemTransferDetailsPage);
