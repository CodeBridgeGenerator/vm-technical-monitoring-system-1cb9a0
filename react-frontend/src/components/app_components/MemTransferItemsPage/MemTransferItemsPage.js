...
import MemTransferItemsDatatable from "./MemTransferItemsDataTable";
...
import MemTransferItemsFakerDialogComponent from "./MemTransferItemsFakerDialogComponent";
import MemTransferItemsSeederDialogComponent from "./MemTransferItemsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MemTransferItemsPage = (props) => {
    ...
    const filename = "memTransferItems.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("memTransferItems");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , transferDate : urlParams.singleMemTransferDetailsId ,part : urlParams.singleMemWarehousePartsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "transferDate",
                        service : "mem_transfer_details",
                        select:["transferDate"]
                    },{
                        path : "part",
                        service : "mem_warehouse_parts",
                        select:["part"]
                    }
            ] }})
            .then((res) => {
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Mem Transfer Items", type: "error", message: error.message || "Failed get Mem Transfer Items" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("memTransferItems").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Mem Transfer Items", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("memTransferItems").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Mem Transfer Items",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/memTransferItems/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Mem Transfer Items </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="memTransferItems-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="memTransferItems-datatable">
                    <MemTransferItemsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MemTransferItemsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MemTransferItemsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MemTransferItemsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MemTransferItemsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MemTransferItemsPage);
