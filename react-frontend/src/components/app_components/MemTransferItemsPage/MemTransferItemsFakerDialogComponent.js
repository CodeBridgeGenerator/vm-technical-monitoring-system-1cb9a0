...
import memTransferItemsFakerFactory from "./memTransferItemsFakerFactory";
...
const MemTransferItemsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [transferDateIds, settransferDateIds] = useState();
const [partIds, setpartIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("memTransferDetails")
                        .find({ query: { $select: ["_id","transferDate"] } })
                        .then((data) => {
                        settransferDateIds(data.data);
                        });
client.service("memWarehouseParts")
                        .find({ query: { $select: ["_id","part"] } })
                        .then((data) => {
                        setpartIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = memTransferItemsFakerFactory(props.user,fakerCount,transferDateIds,partIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memTransferItems").create(elem));
...
export default connect(mapState, mapDispatch)(MemTransferItemsFakerDialogComponent);
