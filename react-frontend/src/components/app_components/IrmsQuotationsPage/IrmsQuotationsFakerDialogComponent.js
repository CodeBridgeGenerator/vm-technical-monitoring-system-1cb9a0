...
import irmsQuotationsFakerFactory from "./irmsQuotationsFakerFactory";
...
const IrmsQuotationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = irmsQuotationsFakerFactory(props.user,fakerCount,salesOrderIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("irmsQuotations").create(elem));
...
export default connect(mapState, mapDispatch)(IrmsQuotationsFakerDialogComponent);
