...
import IrmsQuotationsDatatable from "./IrmsQuotationsDataTable";
...
import IrmsQuotationsFakerDialogComponent from "./IrmsQuotationsFakerDialogComponent";
import IrmsQuotationsSeederDialogComponent from "./IrmsQuotationsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IrmsQuotationsPage = (props) => {
    ...
    const filename = "irmsQuotations.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("irmsQuotations");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , salesOrder : urlParams.singleCustomerSalesOrdersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "IRMS Quotations", type: "error", message: error.message || "Failed get IRMS Quotations" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("irmsQuotations").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "IRMS Quotations", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("irmsQuotations").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "IRMS Quotations",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/irmsQuotations/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="irmsQuotations-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="irmsQuotations-datatable">
                    <IrmsQuotationsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IrmsQuotationsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IrmsQuotationsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IrmsQuotationsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IrmsQuotationsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IrmsQuotationsPage);
