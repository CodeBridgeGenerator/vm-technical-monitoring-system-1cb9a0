...
import memWarehousesFakerFactory from "./memWarehousesFakerFactory";
...
const MemWarehousesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = memWarehousesFakerFactory(props.user,fakerCount,ownershipIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memWarehouses").create(elem));
...
export default connect(mapState, mapDispatch)(MemWarehousesFakerDialogComponent);
