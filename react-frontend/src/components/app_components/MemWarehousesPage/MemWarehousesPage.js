...
import MemWarehousesDatatable from "./MemWarehousesDataTable";
...
import MemWarehousesFakerDialogComponent from "./MemWarehousesFakerDialogComponent";
import MemWarehousesSeederDialogComponent from "./MemWarehousesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MemWarehousesPage = (props) => {
    ...
    const filename = "memWarehouses.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("memWarehouses");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , ownership : urlParams.singleBranchesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Mem Warehouses", type: "error", message: error.message || "Failed get Mem Warehouses" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("memWarehouses").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Mem Warehouses", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("memWarehouses").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Mem Warehouses",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/memWarehouses/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="memWarehouses-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="memWarehouses-datatable">
                    <MemWarehousesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MemWarehousesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MemWarehousesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MemWarehousesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MemWarehousesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MemWarehousesPage);
