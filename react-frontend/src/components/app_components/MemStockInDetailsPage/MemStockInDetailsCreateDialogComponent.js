...
const [warehouse, setWarehouse] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            part: _entity?.part?._id,pricing: _entity?.pricing,quantity: _entity?.quantity,purchaseDate: _entity?.purchaseDate,partDescription: _entity?.partDescription,poNumber: _entity?.poNumber,doNumber: _entity?.doNumber,category: _entity?.category,unitOfMeasurement: _entity?.unitOfMeasurement,conditionOfTerms: _entity?.conditionOfTerms,warehouse: _entity?.warehouse?._id,
