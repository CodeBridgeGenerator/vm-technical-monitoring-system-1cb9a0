...
import memStockInDetailsFakerFactory from "./memStockInDetailsFakerFactory";
...
const MemStockInDetailsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [vmCodeIds, setvmCodeIds] = useState();
const [warehouseIds, setwarehouseIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("atlasMachines")
                        .find({ query: { $select: ["_id","vendingMachineCode"] } })
                        .then((data) => {
                        setvmCodeIds(data.data);
                        });
client.service("memWarehouses")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setwarehouseIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = memStockInDetailsFakerFactory(props.user,fakerCount,vmCodeIds,warehouseIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memStockInDetails").create(elem));
...
export default connect(mapState, mapDispatch)(MemStockInDetailsFakerDialogComponent);
