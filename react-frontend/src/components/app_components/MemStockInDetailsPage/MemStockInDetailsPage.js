...
import MemStockInDetailsDatatable from "./MemStockInDetailsDataTable";
import MemStockInDetailsEditDialogComponent from "./MemStockInDetailsEditDialogComponent";
import MemStockInDetailsCreateDialogComponent from "./MemStockInDetailsCreateDialogComponent";
import MemStockInDetailsFakerDialogComponent from "./MemStockInDetailsFakerDialogComponent";
import MemStockInDetailsSeederDialogComponent from "./MemStockInDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MemStockInDetailsPage = (props) => {
    ...
    const filename = "memStockInDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("memStockInDetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , vmCode : urlParams.singleAtlasMachinesId ,warehouse : urlParams.singleMemWarehousesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "vmCode",
                        service : "atlas_machines",
                        select:["vendingMachineCode"]
                    },{
                        path : "warehouse",
                        service : "mem_warehouses",
                        select:["name"]
                    }
            ] }})
            .then((res) => {
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Mem Stock In Details", type: "error", message: error.message || "Failed get Mem Stock In Details" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("memStockInDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Mem Stock In Details", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("memStockInDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Mem Stock In Details",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/memStockInDetails/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Mem Stock In Details </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="memStockInDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="memStockInDetails-datatable">
                    <MemStockInDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MemStockInDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MemStockInDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MemStockInDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MemStockInDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MemStockInDetailsPage);
