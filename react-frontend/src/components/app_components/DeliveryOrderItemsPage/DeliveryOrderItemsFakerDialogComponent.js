...
import deliveryOrderItemsFakerFactory from "./deliveryOrderItemsFakerFactory";
...
const DeliveryOrderItemsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [deliveryOrdersIds, setdeliveryOrdersIds] = useState();
const [partIds, setpartIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("irmsDeliveryOrders")
                        .find({ query: { $select: ["_id","deliveryOrderId"] } })
                        .then((data) => {
                        setdeliveryOrdersIds(data.data);
                        });
client.service("partsMaster")
    ...
    const onRun = async () => {
        let fakeData = deliveryOrderItemsFakerFactory(props.user,fakerCount,deliveryOrdersIds,partIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("deliveryOrderItems").create(elem));
...
export default connect(mapState, mapDispatch)(DeliveryOrderItemsFakerDialogComponent);
