...
import DeliveryOrderItemsDatatable from "./DeliveryOrderItemsDataTable";
...
import DeliveryOrderItemsFakerDialogComponent from "./DeliveryOrderItemsFakerDialogComponent";
import DeliveryOrderItemsSeederDialogComponent from "./DeliveryOrderItemsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const DeliveryOrderItemsPage = (props) => {
    ...
    const filename = "deliveryOrderItems.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("deliveryOrderItems");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , deliveryOrders : urlParams.singleIrmsDeliveryOrdersId ,part : urlParams.singlePartsMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Delivery Order Items", type: "error", message: error.message || "Failed get Delivery Order Items" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("deliveryOrderItems").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Delivery Order Items", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("deliveryOrderItems").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/deliveryOrderItems/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Delivery Order Items </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="deliveryOrderItems-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="deliveryOrderItems-datatable">
                    <DeliveryOrderItemsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <DeliveryOrderItemsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DeliveryOrderItemsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <DeliveryOrderItemsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DeliveryOrderItemsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(DeliveryOrderItemsPage);
