...
import technicianChecksFakerFactory from "./technicianChecksFakerFactory";
...
const TechnicianChecksFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [technicianCheckListIdIds, settechnicianCheckListIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("technicianChecklists")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        settechnicianCheckListIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = technicianChecksFakerFactory(props.user,fakerCount,technicianCheckListIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("technicianChecks").create(elem));
...
export default connect(mapState, mapDispatch)(TechnicianChecksFakerDialogComponent);
