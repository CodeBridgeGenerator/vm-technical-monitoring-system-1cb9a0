...
import TechnicianChecksDatatable from "./TechnicianChecksDataTable";
import TechnicianChecksEditDialogComponent from "./TechnicianChecksEditDialogComponent";
import TechnicianChecksCreateDialogComponent from "./TechnicianChecksCreateDialogComponent";
import TechnicianChecksFakerDialogComponent from "./TechnicianChecksFakerDialogComponent";
import TechnicianChecksSeederDialogComponent from "./TechnicianChecksSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const TechnicianChecksPage = (props) => {
    ...
    const filename = "technicianChecks.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("technicianChecks");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("technicianChecks")
            .find({ query: { $limit: 10000 , technicianCheckListId : urlParams.singleTechnicianChecklistsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "technicianCheckListId",
                        service : "technician_checklists",
                        select:["name"]
                    }
            ] }})
            .then((res) => {
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Technician Checks", type: "error", message: error.message || "Failed get Technician Checks" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("technicianChecks").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Technician Checks", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("technicianChecks").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/technicianChecks/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Technician Checks </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="technicianChecks-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="technicianChecks-datatable">
                    <TechnicianChecksDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <TechnicianChecksEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TechnicianChecksCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <TechnicianChecksFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <TechnicianChecksSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(TechnicianChecksPage);
