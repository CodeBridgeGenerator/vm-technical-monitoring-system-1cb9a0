...
import machineMasterFakerFactory from "./machineMasterFakerFactory";
...
const MachineMasterFakerDialogComponent = (props) => {
...
client.service("vendingMachines")
    ...
    const onRun = async () => {
        let fakeData = machineMasterFakerFactory(props.user,fakerCount,ownershipIds,vendingMachineTypeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("machineMaster").create(elem));
...
export default connect(mapState, mapDispatch)(MachineMasterFakerDialogComponent);
