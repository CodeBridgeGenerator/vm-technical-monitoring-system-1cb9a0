...
const [vendingMachineType, setVendingMachineType] = useState([])
    ...
    const onSave = async () => {
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Machine Master updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Machine Master" });
        }
        setLoading(false);
    };
...
const vendingMachineTypeOptions = vendingMachineType.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Machine Master" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="serialNumber" className="w-full mb-3 p-inputtext-sm" value={_entity?.serialNumber} onChange={(e) => setValByKey("serialNumber", e.value)}  />
            </span>
            <small className="p-error">
