...
import externalTicketsFakerFactory from "./externalTicketsFakerFactory";
...
const ExternalTicketsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = externalTicketsFakerFactory(props.user,fakerCount,machineIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("externalTickets").create(elem));
...
export default connect(mapState, mapDispatch)(ExternalTicketsFakerDialogComponent);
