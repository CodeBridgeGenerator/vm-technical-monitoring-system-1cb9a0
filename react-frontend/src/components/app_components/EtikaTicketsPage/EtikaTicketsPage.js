...
import EtikaTicketsDatatable from "./EtikaTicketsDataTable";
...
import EtikaTicketsFakerDialogComponent from "./EtikaTicketsFakerDialogComponent";
import EtikaTicketsSeederDialogComponent from "./EtikaTicketsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const EtikaTicketsPage = (props) => {
    ...
    const filename = "etikaTickets.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("etikaTickets");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , machineId : urlParams.singleMachineMasterId ,etikaRequestor : urlParams.singleProfilesId ,assignedSupervisor : urlParams.singleProfilesId ,assignedTechnician : urlParams.singleProfilesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Etika Tickets", type: "error", message: error.message || "Failed get Etika Tickets" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("etikaTickets").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Etika Tickets", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("etikaTickets").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/etikaTickets/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="etikaTickets-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="etikaTickets-datatable">
                    <EtikaTicketsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <EtikaTicketsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <EtikaTicketsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <EtikaTicketsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <EtikaTicketsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(EtikaTicketsPage);
