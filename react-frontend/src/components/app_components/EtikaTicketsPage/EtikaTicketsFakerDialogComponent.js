...
import etikaTicketsFakerFactory from "./etikaTicketsFakerFactory";
...
const EtikaTicketsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [machineIdIds, setmachineIdIds] = useState();
const [etikaRequestorIds, setetikaRequestorIds] = useState();
const [assignedSupervisorIds, setassignedSupervisorIds] = useState();
const [assignedTechnicianIds, setassignedTechnicianIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("machineMaster")
                        .find({ query: { $select: ["_id","serialNumber"] } })
                        .then((data) => {
...
client.service("profiles")
                        ...
                        .then((data) => {
                        setetikaRequestorIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setassignedSupervisorIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setassignedTechnicianIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = etikaTicketsFakerFactory(props.user,fakerCount,machineIdIds,etikaRequestorIds,assignedSupervisorIds,assignedTechnicianIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("etikaTickets").create(elem));
...
export default connect(mapState, mapDispatch)(EtikaTicketsFakerDialogComponent);
