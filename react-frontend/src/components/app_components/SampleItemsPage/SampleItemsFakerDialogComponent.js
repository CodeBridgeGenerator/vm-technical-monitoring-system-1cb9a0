...
import sampleItemsFakerFactory from "./sampleItemsFakerFactory";
...
const SampleItemsFakerDialogComponent = (props) => {
...
client.service("partsMaster")
    ...
    const onRun = async () => {
        let fakeData = sampleItemsFakerFactory(props.user,fakerCount,associatedNoIds,partIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sampleItems").create(elem));
...
export default connect(mapState, mapDispatch)(SampleItemsFakerDialogComponent);
