...
const SampleItemsCreateDialogComponent = (props) => {
...
const [part, setPart] = useState([])
    ...
    const onSave = async () => {
        ...
        const result = await client.service("sampleItems").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info SampleItems updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in SampleItems" });
        }
        setLoading(false);
    };
    ...
    useEffect(() => {
                    // on mount sampleDetails
                    client
                        .service("sampleDetails")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleSampleDetailsId } })
                        .then((res) => {
...
const partOptions = part.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create SampleItems" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="sampleItems-create-dialog-component">
            <div className="col-12 md:col-6 field">
            <span className="align-items-center">
                <label htmlFor="associatedNo">Associated No:</label>
...
export default connect(mapState, mapDispatch)(SampleItemsCreateDialogComponent);
