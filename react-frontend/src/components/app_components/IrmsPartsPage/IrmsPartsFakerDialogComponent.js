...
import irmsPartsFakerFactory from "./irmsPartsFakerFactory";
...
const IrmsPartsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = irmsPartsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("irmsParts").create(elem));
...
export default connect(mapState, mapDispatch)(IrmsPartsFakerDialogComponent);
