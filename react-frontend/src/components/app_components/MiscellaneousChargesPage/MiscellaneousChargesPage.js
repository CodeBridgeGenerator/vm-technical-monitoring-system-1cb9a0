...
import MiscellaneousChargesDatatable from "./MiscellaneousChargesDataTable";
import MiscellaneousChargesEditDialogComponent from "./MiscellaneousChargesEditDialogComponent";
import MiscellaneousChargesCreateDialogComponent from "./MiscellaneousChargesCreateDialogComponent";
import MiscellaneousChargesFakerDialogComponent from "./MiscellaneousChargesFakerDialogComponent";
import MiscellaneousChargesSeederDialogComponent from "./MiscellaneousChargesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MiscellaneousChargesPage = (props) => {
    ...
    const filename = "miscellaneousCharges.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("miscellaneousCharges");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("miscellaneousCharges")
            .find({ query: { $limit: 10000 , quotationNo : urlParams.singleIrmsQuotationsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "MiscellaneousCharges", type: "error", message: error.message || "Failed get MiscellaneousCharges" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("miscellaneousCharges").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "MiscellaneousCharges", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("miscellaneousCharges").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/miscellaneousCharges/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>MiscellaneousCharges </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="miscellaneousCharges-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="miscellaneousCharges-datatable">
                    <MiscellaneousChargesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MiscellaneousChargesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MiscellaneousChargesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MiscellaneousChargesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MiscellaneousChargesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MiscellaneousChargesPage);
