...
import miscellaneousChargesFakerFactory from "./miscellaneousChargesFakerFactory";
...
const MiscellaneousChargesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [quotationNoIds, setquotationNoIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = miscellaneousChargesFakerFactory(props.user,fakerCount,quotationNoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("miscellaneousCharges").create(elem));
...
export default connect(mapState, mapDispatch)(MiscellaneousChargesFakerDialogComponent);
