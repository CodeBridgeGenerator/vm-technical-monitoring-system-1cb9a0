...
import partsMasterFakerFactory from "./partsMasterFakerFactory";
...
const PartsMasterFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = partsMasterFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("partsMaster").create(elem));
...
export default connect(mapState, mapDispatch)(PartsMasterFakerDialogComponent);
