...
import warantyPeriodDetailsFakerFactory from "./warantyPeriodDetailsFakerFactory";
...
const WarantyPeriodDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = warantyPeriodDetailsFakerFactory(props.user,fakerCount,quotationIndexIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("warantyPeriodDetails").create(elem));
...
export default connect(mapState, mapDispatch)(WarantyPeriodDetailsFakerDialogComponent);
