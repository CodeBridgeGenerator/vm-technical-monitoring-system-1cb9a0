...
import WarantyPeriodDetailsDatatable from "./WarantyPeriodDetailsDataTable";
import WarantyPeriodDetailsEditDialogComponent from "./WarantyPeriodDetailsEditDialogComponent";
import WarantyPeriodDetailsCreateDialogComponent from "./WarantyPeriodDetailsCreateDialogComponent";
import WarantyPeriodDetailsFakerDialogComponent from "./WarantyPeriodDetailsFakerDialogComponent";
import WarantyPeriodDetailsSeederDialogComponent from "./WarantyPeriodDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const WarantyPeriodDetailsPage = (props) => {
    ...
    const filename = "warantyPeriodDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("warantyPeriodDetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Waranty Period", type: "error", message: error.message || "Failed get Waranty Period" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("warantyPeriodDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Waranty Period", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("warantyPeriodDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Waranty Period",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/warantyPeriodDetails/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="warantyPeriodDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="warantyPeriodDetails-datatable">
                    <WarantyPeriodDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <WarantyPeriodDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <WarantyPeriodDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <WarantyPeriodDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <WarantyPeriodDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(WarantyPeriodDetailsPage);
