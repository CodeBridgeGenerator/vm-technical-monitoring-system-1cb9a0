...
const WarantyPeriodDetailsCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.period)) {
                error["period"] = `Period field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,period: _entity?.period,quotationIndex: _entity?.quotationIndex?._id,
        ...
        const result = await client.service("warantyPeriodDetails").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Waranty Period updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Waranty Period" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Waranty Period" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="warantyPeriodDetails-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="period" className="w-full mb-3 p-inputtext-sm" value={_entity?.period} onChange={(e) => setValByKey("period", e.target.value)}  required  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                <label htmlFor="quotationIndex">QuotationIndex:</label>
...
export default connect(mapState, mapDispatch)(WarantyPeriodDetailsCreateDialogComponent);
