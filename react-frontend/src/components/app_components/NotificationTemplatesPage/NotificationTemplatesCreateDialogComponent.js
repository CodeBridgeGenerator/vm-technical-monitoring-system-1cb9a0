...
const NotificationTemplatesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,title: _entity?.title,body: _entity?.body,image: _entity?.image,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Notification Templates created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Notification Templates" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Notification Templates" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            style={{ maxWidth: "55vw" }} role="notificationTemplates-create-dialog-component">
