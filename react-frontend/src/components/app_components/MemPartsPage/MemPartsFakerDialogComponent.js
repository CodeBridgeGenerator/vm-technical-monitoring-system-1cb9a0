...
import memPartsFakerFactory from "./memPartsFakerFactory";
...
const MemPartsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = memPartsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memParts").create(elem));
...
export default connect(mapState, mapDispatch)(MemPartsFakerDialogComponent);
