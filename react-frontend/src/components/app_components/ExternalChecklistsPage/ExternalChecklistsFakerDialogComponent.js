...
import externalChecklistsFakerFactory from "./externalChecklistsFakerFactory";
...
const ExternalChecklistsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = externalChecklistsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("externalChecklists").create(elem));
...
export default connect(mapState, mapDispatch)(ExternalChecklistsFakerDialogComponent);
