...
import PartsMasterRawDatatable from "./PartsMasterRawDataTable";
...
import PartsMasterRawFakerDialogComponent from "./PartsMasterRawFakerDialogComponent";
import PartsMasterRawSeederDialogComponent from "./PartsMasterRawSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const PartsMasterRawPage = (props) => {
    ...
    const filename = "partsMasterRaw.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("partsMasterRaw");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "PartsMasterRaw", type: "error", message: error.message || "Failed get PartsMasterRaw" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("partsMasterRaw").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "PartsMasterRaw", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("partsMasterRaw").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "PartsMasterRaw",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/partsMasterRaw/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="partsMasterRaw-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="partsMasterRaw-datatable">
                    <PartsMasterRawDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <PartsMasterRawEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PartsMasterRawCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <PartsMasterRawFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PartsMasterRawSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PartsMasterRawPage);
