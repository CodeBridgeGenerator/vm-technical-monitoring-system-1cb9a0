...
import partsMasterRawFakerFactory from "./partsMasterRawFakerFactory";
...
const PartsMasterRawFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = partsMasterRawFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("partsMasterRaw").create(elem));
...
export default connect(mapState, mapDispatch)(PartsMasterRawFakerDialogComponent);
