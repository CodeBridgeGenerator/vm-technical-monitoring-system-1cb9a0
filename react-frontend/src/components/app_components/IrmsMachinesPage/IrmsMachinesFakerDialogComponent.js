...
import irmsMachinesFakerFactory from "./irmsMachinesFakerFactory";
...
const IrmsMachinesFakerDialogComponent = (props) => {
...
client.service("vendingMachines")
    ...
    const onRun = async () => {
        let fakeData = irmsMachinesFakerFactory(props.user,fakerCount,ownershipIds,vendingMachineTypeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("irmsMachines").create(elem));
...
export default connect(mapState, mapDispatch)(IrmsMachinesFakerDialogComponent);
