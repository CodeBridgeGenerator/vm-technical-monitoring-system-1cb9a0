...
import ExternalPartRequestsDatatable from "./ExternalPartRequestsDataTable";
import ExternalPartRequestsEditDialogComponent from "./ExternalPartRequestsEditDialogComponent";
import ExternalPartRequestsCreateDialogComponent from "./ExternalPartRequestsCreateDialogComponent";
import ExternalPartRequestsFakerDialogComponent from "./ExternalPartRequestsFakerDialogComponent";
import ExternalPartRequestsSeederDialogComponent from "./ExternalPartRequestsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ExternalPartRequestsPage = (props) => {
    ...
    const filename = "externalPartRequests.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("externalPartRequests");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , partName : urlParams.singlePartsMasterId ,externalTicket : urlParams.singleExternalTicketsId ,technician : urlParams.singleProfilesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "partName",
                        service : "parts_master",
                        select:["itemNo"]
                    },{
                        path : "externalTicket",
                        service : "external_tickets",
                        select:["machineId"]
                    },{
                        path : "technician",
                        service : "profiles",
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("externalPartRequests").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "ExternalPartRequests", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("externalPartRequests").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "ExternalPartRequests",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/externalPartRequests/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>ExternalPartRequests </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="externalPartRequests-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="externalPartRequests-datatable">
                    <ExternalPartRequestsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ExternalPartRequestsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ExternalPartRequestsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ExternalPartRequestsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ExternalPartRequestsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ExternalPartRequestsPage);
