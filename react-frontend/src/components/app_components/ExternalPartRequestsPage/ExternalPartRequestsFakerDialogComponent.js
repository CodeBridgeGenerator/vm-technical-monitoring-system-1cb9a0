...
import externalPartRequestsFakerFactory from "./externalPartRequestsFakerFactory";
...
const ExternalPartRequestsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [partNameIds, setpartNameIds] = useState();
const [externalTicketIds, setexternalTicketIds] = useState();
const [technicianIds, settechnicianIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("partsMaster")
                        ...
                        .then((data) => {
                        setpartNameIds(data.data);
                        });
client.service("externalTickets")
                        .find({ query: { $select: ["_id","machineId"] } })
                        .then((data) => {
                        setexternalTicketIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        settechnicianIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = externalPartRequestsFakerFactory(props.user,fakerCount,partNameIds,externalTicketIds,technicianIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("externalPartRequests").create(elem));
...
export default connect(mapState, mapDispatch)(ExternalPartRequestsFakerDialogComponent);
