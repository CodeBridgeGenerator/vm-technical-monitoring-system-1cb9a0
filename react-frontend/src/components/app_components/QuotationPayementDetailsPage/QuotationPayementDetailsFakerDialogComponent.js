...
import quotationPayementDetailsFakerFactory from "./quotationPayementDetailsFakerFactory";
...
const QuotationPayementDetailsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [quotationIndexIds, setquotationIndexIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("irmsQuotations")
                        ...
                        .then((data) => {
                        setquotationIndexIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = quotationPayementDetailsFakerFactory(props.user,fakerCount,quotationIndexIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("quotationPayementDetails").create(elem));
...
export default connect(mapState, mapDispatch)(QuotationPayementDetailsFakerDialogComponent);
