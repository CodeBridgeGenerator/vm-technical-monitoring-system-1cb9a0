...
import QuotationPayementDetailsDatatable from "./QuotationPayementDetailsDataTable";
import QuotationPayementDetailsEditDialogComponent from "./QuotationPayementDetailsEditDialogComponent";
import QuotationPayementDetailsCreateDialogComponent from "./QuotationPayementDetailsCreateDialogComponent";
import QuotationPayementDetailsFakerDialogComponent from "./QuotationPayementDetailsFakerDialogComponent";
import QuotationPayementDetailsSeederDialogComponent from "./QuotationPayementDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const QuotationPayementDetailsPage = (props) => {
    ...
    const filename = "quotationPayementDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("quotationPayementDetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , quotationIndex : urlParams.singleIrmsQuotationsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "QuotationPayementDetails", type: "error", message: error.message || "Failed get QuotationPayementDetails" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("quotationPayementDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "QuotationPayementDetails", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("quotationPayementDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "QuotationPayementDetails",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/quotationPayementDetails/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>QuotationPayementDetails </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="quotationPayementDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="quotationPayementDetails-datatable">
                    <QuotationPayementDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <QuotationPayementDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <QuotationPayementDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <QuotationPayementDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <QuotationPayementDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(QuotationPayementDetailsPage);
