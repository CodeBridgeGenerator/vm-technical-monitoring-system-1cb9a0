...
import stockOutDetailsFakerFactory from "./stockOutDetailsFakerFactory";
...
const StockOutDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stockOutDetailsFakerFactory(props.user,fakerCount,partNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stockOutDetails").create(elem));
...
export default connect(mapState, mapDispatch)(StockOutDetailsFakerDialogComponent);
