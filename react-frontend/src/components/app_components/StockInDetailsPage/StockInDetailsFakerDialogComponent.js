...
import stockInDetailsFakerFactory from "./stockInDetailsFakerFactory";
...
const StockInDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stockInDetailsFakerFactory(props.user,fakerCount,warehouseIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stockInDetails").create(elem));
...
export default connect(mapState, mapDispatch)(StockInDetailsFakerDialogComponent);
