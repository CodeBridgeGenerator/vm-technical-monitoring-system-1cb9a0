...
import DisposalDetailsDatatable from "./DisposalDetailsDataTable";
import DisposalDetailsEditDialogComponent from "./DisposalDetailsEditDialogComponent";
import DisposalDetailsCreateDialogComponent from "./DisposalDetailsCreateDialogComponent";
import DisposalDetailsFakerDialogComponent from "./DisposalDetailsFakerDialogComponent";
import DisposalDetailsSeederDialogComponent from "./DisposalDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const DisposalDetailsPage = (props) => {
    ...
    const filename = "disposalDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("disposalDetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , sourceWarehouse : urlParams.singleWarehouseMasterId ,partNumber : urlParams.singlePartsMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Disposal Details", type: "error", message: error.message || "Failed get Disposal Details" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("disposalDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Disposal Details", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("disposalDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/disposalDetails/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="disposalDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="disposalDetails-datatable">
                    <DisposalDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <DisposalDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DisposalDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <DisposalDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DisposalDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(DisposalDetailsPage);
