...
import disposalDetailsFakerFactory from "./disposalDetailsFakerFactory";
...
const DisposalDetailsFakerDialogComponent = (props) => {
...
client.service("partsMaster")
    ...
    const onRun = async () => {
        let fakeData = disposalDetailsFakerFactory(props.user,fakerCount,sourceWarehouseIds,partNumberIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("disposalDetails").create(elem));
...
export default connect(mapState, mapDispatch)(DisposalDetailsFakerDialogComponent);
