...
import supervisorChecklistsFakerFactory from "./supervisorChecklistsFakerFactory";
...
const SupervisorChecklistsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = supervisorChecklistsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("supervisorChecklists").create(elem));
...
export default connect(mapState, mapDispatch)(SupervisorChecklistsFakerDialogComponent);
