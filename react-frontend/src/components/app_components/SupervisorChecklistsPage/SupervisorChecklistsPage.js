...
import SupervisorChecklistsDatatable from "./SupervisorChecklistsDataTable";
...
import SupervisorChecklistsFakerDialogComponent from "./SupervisorChecklistsFakerDialogComponent";
import SupervisorChecklistsSeederDialogComponent from "./SupervisorChecklistsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const SupervisorChecklistsPage = (props) => {
    ...
    const filename = "supervisorChecklists.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("supervisorChecklists");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "SupervisorChecklists", type: "error", message: error.message || "Failed get SupervisorChecklists" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("supervisorChecklists").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "SupervisorChecklists", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("supervisorChecklists").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "SupervisorChecklists",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/supervisorChecklists/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>SupervisorChecklists </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="supervisorChecklists-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="supervisorChecklists-datatable">
                    <SupervisorChecklistsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <SupervisorChecklistsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SupervisorChecklistsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <SupervisorChecklistsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SupervisorChecklistsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(SupervisorChecklistsPage);
