...
import AtlasChecksDatatable from "./AtlasChecksDataTable";
import AtlasChecksEditDialogComponent from "./AtlasChecksEditDialogComponent";
import AtlasChecksCreateDialogComponent from "./AtlasChecksCreateDialogComponent";
import AtlasChecksFakerDialogComponent from "./AtlasChecksFakerDialogComponent";
import AtlasChecksSeederDialogComponent from "./AtlasChecksSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const AtlasChecksPage = (props) => {
    ...
    const filename = "atlasChecks.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("atlasChecks");
    ...
    useEffect(() => {
        ...
        client
            .service("atlasChecks")
            .find({ query: { $limit: 10000 , atlasCheckListId : urlParams.singleAtlasChecklistsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Atlas Checks", type: "error", message: error.message || "Failed get Atlas Checks" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("atlasChecks").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Atlas Checks", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("atlasChecks").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="atlasChecks-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="atlasChecks-datatable">
                    <AtlasChecksDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <AtlasChecksEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AtlasChecksCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <AtlasChecksFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AtlasChecksSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AtlasChecksPage);
