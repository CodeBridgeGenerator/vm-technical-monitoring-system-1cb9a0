...
import atlasChecksFakerFactory from "./atlasChecksFakerFactory";
...
const AtlasChecksFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [atlasCheckListIdIds, setatlasCheckListIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("atlasChecklists")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setatlasCheckListIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = atlasChecksFakerFactory(props.user,fakerCount,atlasCheckListIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("atlasChecks").create(elem));
...
export default connect(mapState, mapDispatch)(AtlasChecksFakerDialogComponent);
