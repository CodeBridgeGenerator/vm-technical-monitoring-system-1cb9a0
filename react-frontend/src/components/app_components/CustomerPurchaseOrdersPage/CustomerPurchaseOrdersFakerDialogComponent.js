...
import customerPurchaseOrdersFakerFactory from "./customerPurchaseOrdersFakerFactory";
...
const CustomerPurchaseOrdersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = customerPurchaseOrdersFakerFactory(props.user,fakerCount,quotationIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customerPurchaseOrders").create(elem));
...
export default connect(mapState, mapDispatch)(CustomerPurchaseOrdersFakerDialogComponent);
