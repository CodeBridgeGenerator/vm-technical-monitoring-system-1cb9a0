...
import CustomerPurchaseOrdersDatatable from "./CustomerPurchaseOrdersDataTable";
import CustomerPurchaseOrdersEditDialogComponent from "./CustomerPurchaseOrdersEditDialogComponent";
import CustomerPurchaseOrdersCreateDialogComponent from "./CustomerPurchaseOrdersCreateDialogComponent";
import CustomerPurchaseOrdersFakerDialogComponent from "./CustomerPurchaseOrdersFakerDialogComponent";
import CustomerPurchaseOrdersSeederDialogComponent from "./CustomerPurchaseOrdersSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const CustomerPurchaseOrdersPage = (props) => {
    ...
    const filename = "customerPurchaseOrders.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("customerPurchaseOrders");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , quotation : urlParams.singleIrmsQuotationsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("customerPurchaseOrders").remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("customerPurchaseOrders").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/customerPurchaseOrders/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="customerPurchaseOrders-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="customerPurchaseOrders-datatable">
                    <CustomerPurchaseOrdersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CustomerPurchaseOrdersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CustomerPurchaseOrdersCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CustomerPurchaseOrdersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CustomerPurchaseOrdersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CustomerPurchaseOrdersPage);
