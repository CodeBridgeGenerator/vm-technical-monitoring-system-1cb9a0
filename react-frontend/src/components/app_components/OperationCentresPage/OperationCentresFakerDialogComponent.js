...
import operationCentresFakerFactory from "./operationCentresFakerFactory";
...
const OperationCentresFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("locationMaster")
                        .find({ query: { $select: ["_id","code"] } })
                        .then((data) => {
                        setcodeIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = operationCentresFakerFactory(props.user,fakerCount,codeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("operationCentres").create(elem));
...
export default connect(mapState, mapDispatch)(OperationCentresFakerDialogComponent);
