...
import externalChecksFakerFactory from "./externalChecksFakerFactory";
...
const ExternalChecksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = externalChecksFakerFactory(props.user,fakerCount,checkListIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("externalChecks").create(elem));
...
export default connect(mapState, mapDispatch)(ExternalChecksFakerDialogComponent);
