...
import ExternalChecksDatatable from "./ExternalChecksDataTable";
import ExternalChecksEditDialogComponent from "./ExternalChecksEditDialogComponent";
import ExternalChecksCreateDialogComponent from "./ExternalChecksCreateDialogComponent";
import ExternalChecksFakerDialogComponent from "./ExternalChecksFakerDialogComponent";
import ExternalChecksSeederDialogComponent from "./ExternalChecksSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ExternalChecksPage = (props) => {
    ...
    const filename = "externalChecks.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("externalChecks");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , checkListId : urlParams.singleExternalChecklistsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "External Checks", type: "error", message: error.message || "Failed get External Checks" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("externalChecks").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "External Checks", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("externalChecks").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/externalChecks/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="externalChecks-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="externalChecks-datatable">
                    <ExternalChecksDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ExternalChecksEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ExternalChecksCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ExternalChecksFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ExternalChecksSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ExternalChecksPage);
