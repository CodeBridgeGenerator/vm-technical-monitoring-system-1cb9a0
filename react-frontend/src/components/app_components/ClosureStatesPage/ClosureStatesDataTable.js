...
const ClosureStatesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.description}</p>
  ...
  const handleHideDialog = () => {
    ...
    return (
        ...
        />
<Column field="status" header="Status" body={pTemplate0} filter={selectedFilterFields.includes("status")} hidden={selectedHideFields?.includes("status")}  sortable style={{ minWidth: "8rem" }} />
<Column field="description" header="Description" body={pTemplate1} filter={selectedFilterFields.includes("description")} hidden={selectedHideFields?.includes("description")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload ClosureStates Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="closureStates"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search ClosureStates" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ClosureStatesDataTable;