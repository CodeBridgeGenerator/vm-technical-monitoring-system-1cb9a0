...
import closureStatesFakerFactory from "./closureStatesFakerFactory";
...
const ClosureStatesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = closureStatesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("closureStates").create(elem));
...
export default connect(mapState, mapDispatch)(ClosureStatesFakerDialogComponent);
