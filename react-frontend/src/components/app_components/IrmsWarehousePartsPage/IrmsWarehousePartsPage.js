...
import IrmsWarehousePartsDatatable from "./IrmsWarehousePartsDataTable";
...
import IrmsWarehousePartsFakerDialogComponent from "./IrmsWarehousePartsFakerDialogComponent";
import IrmsWarehousePartsSeederDialogComponent from "./IrmsWarehousePartsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IrmsWarehousePartsPage = (props) => {
    ...
    const filename = "irmsWarehouseParts.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("irmsWarehouseParts");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , part : urlParams.singleIrmsPartsId ,warehouse : urlParams.singleWarehouseMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Irms Warehouse Parts", type: "error", message: error.message || "Failed get Irms Warehouse Parts" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("irmsWarehouseParts").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Irms Warehouse Parts", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("irmsWarehouseParts").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Irms Warehouse Parts",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/irmsWarehouseParts/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Irms Warehouse Parts </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="irmsWarehouseParts-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="irmsWarehouseParts-datatable">
                    <IrmsWarehousePartsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IrmsWarehousePartsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IrmsWarehousePartsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IrmsWarehousePartsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IrmsWarehousePartsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IrmsWarehousePartsPage);
