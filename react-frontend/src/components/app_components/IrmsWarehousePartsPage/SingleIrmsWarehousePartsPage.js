...
const SingleIrmsWarehousePartsPage = (props) => {
...
const [warehouse, setWarehouse] = useState([]);
    ...
    useEffect(() => {
        ...
        client
            ...
            .get(urlParams.singleIrmsWarehousePartsId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
            ...
            ? res.part.map((elem) => ({ _id: elem._id, itemNo: elem.itemNo }))
            : res.part
                ? [{ _id: res.part._id, itemNo: res.part.itemNo }]
                : [];
        setPart(part);
const warehouse = Array.isArray(res.warehouse)
        ...
        setWarehouse(warehouse);
            ...
            .catch((error) => {
                console.log({ error });
                props.alert({ title: "IrmsWarehouseParts", type: "error", message: error.message || "Failed get irmsWarehouseParts" });
            });
    }, [props,urlParams.singleIrmsWarehousePartsId]);


    const goBack = () => {
        navigate("/app/irmsWarehouseParts");
    };
        ...
        <div className="col-12 flex flex-column align-items-center">
            <div className="col-12">
                ...
                <div className="flex align-items-center">
                    ...
                    <h3 className="m-0">Irms Warehouse Parts</h3>
                    <SplitButton
                ...
                {/* <p>irmsWarehouseParts/{urlParams.singleIrmsWarehousePartsId}</p> */}
            </div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Reordering Point</label><p className="m-0 ml-3" >{Number(_entity?.reorderingPoint)}</p></div>
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Part</label>
                    {part.map((elem) => (
                        <Link key={elem._id} to={`/irmsParts/${elem._id}`}>
                        <div>
                  {" "}
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Warehouse</label>
      ...
      <CommentsSection
        recordId={urlParams.singleIrmsWarehousePartsId}
        user={props.user}
        alert={props.alert}
...
export default connect(mapState, mapDispatch)(SingleIrmsWarehousePartsPage);
