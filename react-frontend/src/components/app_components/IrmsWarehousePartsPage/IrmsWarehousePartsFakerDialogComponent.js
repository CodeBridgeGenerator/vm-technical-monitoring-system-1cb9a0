...
import irmsWarehousePartsFakerFactory from "./irmsWarehousePartsFakerFactory";
...
const IrmsWarehousePartsFakerDialogComponent = (props) => {
...
client.service("warehouseMaster")
    ...
    const onRun = async () => {
        let fakeData = irmsWarehousePartsFakerFactory(props.user,fakerCount,partIds,warehouseIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("irmsWarehouseParts").create(elem));
...
export default connect(mapState, mapDispatch)(IrmsWarehousePartsFakerDialogComponent);
