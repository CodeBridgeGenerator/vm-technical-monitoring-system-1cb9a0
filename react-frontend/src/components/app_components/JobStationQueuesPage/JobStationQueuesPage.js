...
import JobStationQueuesDatatable from "./JobStationQueuesDataTable";
...
import JobStationQueuesFakerDialogComponent from "./JobStationQueuesFakerDialogComponent";
import JobStationQueuesSeederDialogComponent from "./JobStationQueuesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const JobStationQueuesPage = (props) => {
    ...
    const filename = "jobStationQueues.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("jobStationQueues");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , incomingMachineTicketId : urlParams.singleIncomingMachineTicketsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "JobStation Queues", type: "error", message: error.message || "Failed get JobStation Queues" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("jobStationQueues").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "JobStation Queues", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("jobStationQueues").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "JobStation Queues",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/jobStationQueues/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>JobStation Queues </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="jobStationQueues-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="jobStationQueues-datatable">
                    <JobStationQueuesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <JobStationQueuesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <JobStationQueuesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <JobStationQueuesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <JobStationQueuesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(JobStationQueuesPage);
