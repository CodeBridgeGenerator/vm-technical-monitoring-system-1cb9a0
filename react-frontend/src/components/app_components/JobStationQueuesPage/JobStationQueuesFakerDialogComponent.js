...
import jobStationQueuesFakerFactory from "./jobStationQueuesFakerFactory";
...
const JobStationQueuesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [incomingMachineTicketIdIds, setincomingMachineTicketIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("incomingMachineTickets")
                        ...
                        .then((data) => {
                        setincomingMachineTicketIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = jobStationQueuesFakerFactory(props.user,fakerCount,incomingMachineTicketIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobStationQueues").create(elem));
...
export default connect(mapState, mapDispatch)(JobStationQueuesFakerDialogComponent);
