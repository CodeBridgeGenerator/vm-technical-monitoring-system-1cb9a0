...
import IncomingMachineTicketsDatatable from "./IncomingMachineTicketsDataTable";
...
import IncomingMachineTicketsFakerDialogComponent from "./IncomingMachineTicketsFakerDialogComponent";
import IncomingMachineTicketsSeederDialogComponent from "./IncomingMachineTicketsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IncomingMachineTicketsPage = (props) => {
    ...
    const filename = "incomingMachineTickets.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("incomingMachineTickets");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , machineId : urlParams.singleMachineMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Incoming Machine Tickets", type: "error", message: error.message || "Failed get Incoming Machine Tickets" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("incomingMachineTickets").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Incoming Machine Tickets", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("incomingMachineTickets").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Incoming Machine Tickets",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/incomingMachineTickets/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Incoming Machine Tickets </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="incomingMachineTickets-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="incomingMachineTickets-datatable">
                    <IncomingMachineTicketsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IncomingMachineTicketsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IncomingMachineTicketsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IncomingMachineTicketsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IncomingMachineTicketsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IncomingMachineTicketsPage);
