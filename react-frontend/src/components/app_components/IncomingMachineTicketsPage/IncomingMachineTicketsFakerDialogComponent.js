...
import incomingMachineTicketsFakerFactory from "./incomingMachineTicketsFakerFactory";
...
const IncomingMachineTicketsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = incomingMachineTicketsFakerFactory(props.user,fakerCount,machineIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("incomingMachineTickets").create(elem));
...
export default connect(mapState, mapDispatch)(IncomingMachineTicketsFakerDialogComponent);
