...
import memStockOutDetailsFakerFactory from "./memStockOutDetailsFakerFactory";
...
const MemStockOutDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = memStockOutDetailsFakerFactory(props.user,fakerCount,partNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memStockOutDetails").create(elem));
...
export default connect(mapState, mapDispatch)(MemStockOutDetailsFakerDialogComponent);
