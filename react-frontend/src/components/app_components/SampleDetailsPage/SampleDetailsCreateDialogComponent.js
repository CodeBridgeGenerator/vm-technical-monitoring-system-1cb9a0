...
const SampleDetailsCreateDialogComponent = (props) => {
...
const [partNumber, setPartNumber] = useState([])
    ...
    const onSave = async () => {
        ...
        const result = await client.service("sampleDetails").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Sample Details updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Sample Details" });
        }
        setLoading(false);
    };
...
const partNumberOptions = partNumber.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Sample Details" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="sampleDetails-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="affectiveDate"  value={_entity?.affectiveDate ? new Date(_entity?.affectiveDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("affectiveDate", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(SampleDetailsCreateDialogComponent);
