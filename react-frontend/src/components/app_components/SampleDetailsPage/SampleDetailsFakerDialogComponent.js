...
import sampleDetailsFakerFactory from "./sampleDetailsFakerFactory";
...
const SampleDetailsFakerDialogComponent = (props) => {
...
client.service("partsMaster")
    ...
    const onRun = async () => {
        let fakeData = sampleDetailsFakerFactory(props.user,fakerCount,sourceWarehouseIds,partNumberIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sampleDetails").create(elem));
...
export default connect(mapState, mapDispatch)(SampleDetailsFakerDialogComponent);
