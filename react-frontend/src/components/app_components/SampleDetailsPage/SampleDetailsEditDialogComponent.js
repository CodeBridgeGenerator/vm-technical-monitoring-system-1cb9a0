...
const SampleDetailsEditDialogComponent = (props) => {
...
affectiveDate: _entity?.affectiveDate,
        ...
        await client.service("sampleDetails").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info sampleDetails updated successfully" });
...
const partNumberOptions = partNumber.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Sample Details" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="sampleDetails-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="affectiveDate" value={_entity?.affectiveDate ? new Date(_entity?.affectiveDate) : null} onChange={ (e) => setValByKey("affectiveDate", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(SampleDetailsEditDialogComponent);
