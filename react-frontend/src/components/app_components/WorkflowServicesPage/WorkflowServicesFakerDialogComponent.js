...
import workflowServicesFakerFactory from "./workflowServicesFakerFactory";
...
const WorkflowServicesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = workflowServicesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("workflowServices").create(elem));
...
export default connect(mapState, mapDispatch)(WorkflowServicesFakerDialogComponent);
