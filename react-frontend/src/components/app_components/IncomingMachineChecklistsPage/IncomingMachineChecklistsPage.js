...
import IncomingMachineChecklistsDatatable from "./IncomingMachineChecklistsDataTable";
...
import IncomingMachineChecklistsFakerDialogComponent from "./IncomingMachineChecklistsFakerDialogComponent";
import IncomingMachineChecklistsSeederDialogComponent from "./IncomingMachineChecklistsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IncomingMachineChecklistsPage = (props) => {
    ...
    const filename = "incomingMachineChecklists.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("incomingMachineChecklists");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "IncomingMachineChecklists", type: "error", message: error.message || "Failed get IncomingMachineChecklists" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("incomingMachineChecklists").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "IncomingMachineChecklists", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("incomingMachineChecklists").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "IncomingMachineChecklists",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/incomingMachineChecklists/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>IncomingMachineChecklists </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="incomingMachineChecklists-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="incomingMachineChecklists-datatable">
                    <IncomingMachineChecklistsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IncomingMachineChecklistsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IncomingMachineChecklistsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IncomingMachineChecklistsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IncomingMachineChecklistsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IncomingMachineChecklistsPage);
