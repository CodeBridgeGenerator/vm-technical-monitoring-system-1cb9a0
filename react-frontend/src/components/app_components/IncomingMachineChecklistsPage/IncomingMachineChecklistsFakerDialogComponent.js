...
import incomingMachineChecklistsFakerFactory from "./incomingMachineChecklistsFakerFactory";
...
const IncomingMachineChecklistsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = incomingMachineChecklistsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("incomingMachineChecklists").create(elem));
...
export default connect(mapState, mapDispatch)(IncomingMachineChecklistsFakerDialogComponent);
