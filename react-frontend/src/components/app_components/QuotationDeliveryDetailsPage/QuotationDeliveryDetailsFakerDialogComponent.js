...
import quotationDeliveryDetailsFakerFactory from "./quotationDeliveryDetailsFakerFactory";
...
const QuotationDeliveryDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = quotationDeliveryDetailsFakerFactory(props.user,fakerCount,quotationIndexIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("quotationDeliveryDetails").create(elem));
...
export default connect(mapState, mapDispatch)(QuotationDeliveryDetailsFakerDialogComponent);
