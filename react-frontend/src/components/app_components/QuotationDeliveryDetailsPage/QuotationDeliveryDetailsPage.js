...
import QuotationDeliveryDetailsDatatable from "./QuotationDeliveryDetailsDataTable";
...
import QuotationDeliveryDetailsFakerDialogComponent from "./QuotationDeliveryDetailsFakerDialogComponent";
import QuotationDeliveryDetailsSeederDialogComponent from "./QuotationDeliveryDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const QuotationDeliveryDetailsPage = (props) => {
    ...
    const filename = "quotationDeliveryDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("quotationDeliveryDetails");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("quotationDeliveryDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "QuotationDeliveryDetails", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("quotationDeliveryDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "QuotationDeliveryDetails",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/quotationDeliveryDetails/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>QuotationDeliveryDetails </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="quotationDeliveryDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="quotationDeliveryDetails-datatable">
                    <QuotationDeliveryDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <QuotationDeliveryDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <QuotationDeliveryDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <QuotationDeliveryDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <QuotationDeliveryDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(QuotationDeliveryDetailsPage);
