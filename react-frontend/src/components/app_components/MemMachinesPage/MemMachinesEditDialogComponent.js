...
const MemMachinesEditDialogComponent = (props) => {
...
comissionDate: _entity?.comissionDate,
        ...
        await client.service("memMachines").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info memMachines updated successfully" });
...
const vendingMachineTypeOptions = vendingMachineType.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit MEM Machines" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="memMachines-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="comissionDate" value={_entity?.comissionDate ? new Date(_entity?.comissionDate) : null} onChange={ (e) => setValByKey("comissionDate", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(MemMachinesEditDialogComponent);
