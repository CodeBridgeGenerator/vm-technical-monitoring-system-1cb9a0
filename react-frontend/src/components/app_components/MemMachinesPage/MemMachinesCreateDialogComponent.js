...
const MemMachinesCreateDialogComponent = (props) => {
...
const [vendingMachineType, setVendingMachineType] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            ownership: _entity?.ownership?._id,vendingMachineCode: _entity?.vendingMachineCode,modelNo: _entity?.modelNo,serialNumber: _entity?.serialNumber,vendingMachineType: _entity?.vendingMachineType?._id,comissionDate: _entity?.comissionDate,
        ...
        const result = await client.service("memMachines").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info MEM Machines updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in MEM Machines" });
        }
        setLoading(false);
    };
...
const vendingMachineTypeOptions = vendingMachineType.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create MEM Machines" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="memMachines-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="comissionDate"  value={_entity?.comissionDate ? new Date(_entity?.comissionDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("comissionDate", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(MemMachinesCreateDialogComponent);
