...
import memMachinesFakerFactory from "./memMachinesFakerFactory";
...
const MemMachinesFakerDialogComponent = (props) => {
...
client.service("vendingMachines")
    ...
    const onRun = async () => {
        let fakeData = memMachinesFakerFactory(props.user,fakerCount,ownershipIds,vendingMachineTypeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memMachines").create(elem));
...
export default connect(mapState, mapDispatch)(MemMachinesFakerDialogComponent);
