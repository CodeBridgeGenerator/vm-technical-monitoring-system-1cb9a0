...
const AtlasMachinesPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , ownership : urlParams.singleUsersId ,vmType : urlParams.singleVendingMachinesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            ...
            console.log({ error });
            props.alert({ title: "AtlasMachines", type: "error", message: error.message || "Failed delete record" });
        }
    };
