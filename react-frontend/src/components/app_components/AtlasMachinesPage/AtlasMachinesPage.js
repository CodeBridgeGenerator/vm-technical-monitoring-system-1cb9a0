...
import AtlasMachinesDatatable from "./AtlasMachinesDataTable";
import AtlasMachinesEditDialogComponent from "./AtlasMachinesEditDialogComponent";
import AtlasMachinesCreateDialogComponent from "./AtlasMachinesCreateDialogComponent";
import AtlasMachinesFakerDialogComponent from "./AtlasMachinesFakerDialogComponent";
import AtlasMachinesSeederDialogComponent from "./AtlasMachinesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const AtlasMachinesPage = (props) => {
    ...
    const filename = "atlasMachines.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("atlasMachines");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Atlas Machines", type: "error", message: error.message || "Failed get Atlas Machines" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("atlasMachines").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Atlas Machines", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("atlasMachines").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Atlas Machines",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/atlasMachines/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="atlasMachines-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="atlasMachines-datatable">
                    <AtlasMachinesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <AtlasMachinesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AtlasMachinesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <AtlasMachinesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AtlasMachinesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AtlasMachinesPage);
