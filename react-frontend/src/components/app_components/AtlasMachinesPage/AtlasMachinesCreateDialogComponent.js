...
const [vendingMachineType, setVendingMachineType] = useState([])
    ...
    const onSave = async () => {
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Atlas Machines updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Atlas Machines" });
        }
        setLoading(false);
    };
...
const vendingMachineTypeOptions = vendingMachineType.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Atlas Machines" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
