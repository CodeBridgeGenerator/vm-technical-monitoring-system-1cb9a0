...
import atlasMachinesFakerFactory from "./atlasMachinesFakerFactory";
...
const AtlasMachinesFakerDialogComponent = (props) => {
...
client.service("vendingMachines")
    ...
    const onRun = async () => {
        let fakeData = atlasMachinesFakerFactory(props.user,fakerCount,ownershipIds,vendingMachineTypeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("atlasMachines").create(elem));
...
export default connect(mapState, mapDispatch)(AtlasMachinesFakerDialogComponent);
