...
import IncomingMachineChecksDatatable from "./IncomingMachineChecksDataTable";
import IncomingMachineChecksEditDialogComponent from "./IncomingMachineChecksEditDialogComponent";
import IncomingMachineChecksCreateDialogComponent from "./IncomingMachineChecksCreateDialogComponent";
import IncomingMachineChecksFakerDialogComponent from "./IncomingMachineChecksFakerDialogComponent";
import IncomingMachineChecksSeederDialogComponent from "./IncomingMachineChecksSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IncomingMachineChecksPage = (props) => {
    ...
    const filename = "incomingMachineChecks.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("incomingMachineChecks");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("incomingMachineChecks")
            .find({ query: { $limit: 10000 , checkListId : urlParams.singleIncomingMachineChecklistsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "checkListId",
                        service : "incoming_machine_checklists",
                        select:["name"]
                    }
            ] }})
            .then((res) => {
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "IncomingMachineChecks", type: "error", message: error.message || "Failed get IncomingMachineChecks" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("incomingMachineChecks").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "IncomingMachineChecks", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("incomingMachineChecks").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "IncomingMachineChecks",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/incomingMachineChecks/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>IncomingMachineChecks </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="incomingMachineChecks-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="incomingMachineChecks-datatable">
                    <IncomingMachineChecksDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IncomingMachineChecksEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IncomingMachineChecksCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IncomingMachineChecksFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IncomingMachineChecksSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IncomingMachineChecksPage);
