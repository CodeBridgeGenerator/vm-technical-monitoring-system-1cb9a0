...
import incomingMachineChecksFakerFactory from "./incomingMachineChecksFakerFactory";
...
const IncomingMachineChecksFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [checkListIdIds, setcheckListIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("incomingMachineChecklists")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setcheckListIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = incomingMachineChecksFakerFactory(props.user,fakerCount,checkListIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("incomingMachineChecks").create(elem));
...
export default connect(mapState, mapDispatch)(IncomingMachineChecksFakerDialogComponent);
