...
import warehouseMasterFakerFactory from "./warehouseMasterFakerFactory";
...
const WarehouseMasterFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = warehouseMasterFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("warehouseMaster").create(elem));
...
export default connect(mapState, mapDispatch)(WarehouseMasterFakerDialogComponent);
