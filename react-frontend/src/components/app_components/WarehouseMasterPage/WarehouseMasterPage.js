...
import WarehouseMasterDatatable from "./WarehouseMasterDataTable";
import WarehouseMasterEditDialogComponent from "./WarehouseMasterEditDialogComponent";
import WarehouseMasterCreateDialogComponent from "./WarehouseMasterCreateDialogComponent";
import WarehouseMasterFakerDialogComponent from "./WarehouseMasterFakerDialogComponent";
import WarehouseMasterSeederDialogComponent from "./WarehouseMasterSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const WarehouseMasterPage = (props) => {
    ...
    const filename = "warehouseMaster.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("warehouseMaster");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("warehouseMaster").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "WarehouseMaster", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("warehouseMaster").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/warehouseMaster/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="warehouseMaster-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="warehouseMaster-datatable">
                    <WarehouseMasterDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <WarehouseMasterEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <WarehouseMasterCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <WarehouseMasterFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <WarehouseMasterSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(WarehouseMasterPage);
