...
import technicianChecklistsFakerFactory from "./technicianChecklistsFakerFactory";
...
const TechnicianChecklistsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = technicianChecklistsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("technicianChecklists").create(elem));
...
export default connect(mapState, mapDispatch)(TechnicianChecklistsFakerDialogComponent);
