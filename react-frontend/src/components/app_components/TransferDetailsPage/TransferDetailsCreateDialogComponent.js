...
const [partNumber, setPartNumber] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [sourceWarehouse,destinationWarehouse,partNumber], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            sourceWarehouse: _entity?.sourceWarehouse?._id,destinationWarehouse: _entity?.destinationWarehouse?._id,partNumber: _entity?.partNumber?._id,quantity: _entity?.quantity,transferDate: _entity?.transferDate,transferStatus: _entity?.transferStatus,
        ...
        const result = await client.service("transferDetails").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Transfer Details updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Transfer Details" });
        }
        setLoading(false);
    };
...
const partNumberOptions = partNumber.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Transfer Details" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="transferDetails-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="transferDate"  value={_entity?.transferDate ? new Date(_entity?.transferDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("transferDate", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
