...
import transferDetailsFakerFactory from "./transferDetailsFakerFactory";
...
const TransferDetailsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [sourceWarehouseIds, setsourceWarehouseIds] = useState();
const [destinationWarehouseIds, setdestinationWarehouseIds] = useState();
const [partNumberIds, setpartNumberIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("warehouseMaster")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setsourceWarehouseIds(data.data);
                        });
client.service("warehouseMaster")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setdestinationWarehouseIds(data.data);
                        });
client.service("partsMaster")
                        .find({ query: { $select: ["_id","itemNo"] } })
                        .then((data) => {
                        setpartNumberIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = transferDetailsFakerFactory(props.user,fakerCount,sourceWarehouseIds,destinationWarehouseIds,partNumberIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("transferDetails").create(elem));
...
export default connect(mapState, mapDispatch)(TransferDetailsFakerDialogComponent);
