...
import jobStationsFakerFactory from "./jobStationsFakerFactory";
...
const JobStationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = jobStationsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobStations").create(elem));
...
export default connect(mapState, mapDispatch)(JobStationsFakerDialogComponent);
