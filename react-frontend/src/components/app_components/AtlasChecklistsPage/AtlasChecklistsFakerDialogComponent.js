...
import atlasChecklistsFakerFactory from "./atlasChecklistsFakerFactory";
...
const AtlasChecklistsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = atlasChecklistsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("atlasChecklists").create(elem));
...
export default connect(mapState, mapDispatch)(AtlasChecklistsFakerDialogComponent);
