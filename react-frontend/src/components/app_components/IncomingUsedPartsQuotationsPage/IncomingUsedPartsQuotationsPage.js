...
import IncomingUsedPartsQuotationsDatatable from "./IncomingUsedPartsQuotationsDataTable";
import IncomingUsedPartsQuotationsEditDialogComponent from "./IncomingUsedPartsQuotationsEditDialogComponent";
import IncomingUsedPartsQuotationsCreateDialogComponent from "./IncomingUsedPartsQuotationsCreateDialogComponent";
import IncomingUsedPartsQuotationsFakerDialogComponent from "./IncomingUsedPartsQuotationsFakerDialogComponent";
import IncomingUsedPartsQuotationsSeederDialogComponent from "./IncomingUsedPartsQuotationsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IncomingUsedPartsQuotationsPage = (props) => {
    ...
    const filename = "incomingUsedPartsQuotations.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("incomingUsedPartsQuotations");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , quotation : urlParams.singleIrmsQuotationsId ,incomingMachineTicket : urlParams.singleIncomingMachineTicketsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "quotation",
                        service : "irms_quotations",
                        select:["quotationIndex"]
                    },{
                        path : "incomingMachineTicket",
                        service : "incoming_machine_tickets",
                        select:["machineId"]
                    }
            ] }})
            .then((res) => {
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Incoming Used Parts Quotations", type: "error", message: error.message || "Failed get Incoming Used Parts Quotations" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("incomingUsedPartsQuotations").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Incoming Used Parts Quotations", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("incomingUsedPartsQuotations").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Incoming Used Parts Quotations",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/incomingUsedPartsQuotations/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Incoming Used Parts Quotations </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="incomingUsedPartsQuotations-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="incomingUsedPartsQuotations-datatable">
                    <IncomingUsedPartsQuotationsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IncomingUsedPartsQuotationsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IncomingUsedPartsQuotationsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IncomingUsedPartsQuotationsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IncomingUsedPartsQuotationsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IncomingUsedPartsQuotationsPage);
