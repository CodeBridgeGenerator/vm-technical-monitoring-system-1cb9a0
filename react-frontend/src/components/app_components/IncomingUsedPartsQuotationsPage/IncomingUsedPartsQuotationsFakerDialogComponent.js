...
import incomingUsedPartsQuotationsFakerFactory from "./incomingUsedPartsQuotationsFakerFactory";
...
const IncomingUsedPartsQuotationsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [quotationIds, setquotationIds] = useState();
const [incomingMachineTicketIds, setincomingMachineTicketIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("irmsQuotations")
                        .find({ query: { $select: ["_id","quotationIndex"] } })
                        .then((data) => {
...
client.service("incomingMachineTickets")
                        ...
                        .then((data) => {
                        setincomingMachineTicketIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = incomingUsedPartsQuotationsFakerFactory(props.user,fakerCount,quotationIds,incomingMachineTicketIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("incomingUsedPartsQuotations").create(elem));
...
export default connect(mapState, mapDispatch)(IncomingUsedPartsQuotationsFakerDialogComponent);
