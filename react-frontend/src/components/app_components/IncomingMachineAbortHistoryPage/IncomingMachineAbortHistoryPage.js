...
import IncomingMachineAbortHistoryDatatable from "./IncomingMachineAbortHistoryDataTable";
...
import IncomingMachineAbortHistoryFakerDialogComponent from "./IncomingMachineAbortHistoryFakerDialogComponent";
import IncomingMachineAbortHistorySeederDialogComponent from "./IncomingMachineAbortHistorySeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const IncomingMachineAbortHistoryPage = (props) => {
    ...
    const filename = "incomingMachineAbortHistory.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("incomingMachineAbortHistory");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , ticketId : urlParams.singleIncomingMachineTicketsId ,abortedBy : urlParams.singleUsersId ,machineId : urlParams.singleMachineMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Abort History ", type: "error", message: error.message || "Failed get Abort History " });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("incomingMachineAbortHistory").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Abort History ", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("incomingMachineAbortHistory").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/incomingMachineAbortHistory/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="incomingMachineAbortHistory-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="incomingMachineAbortHistory-datatable">
                    <IncomingMachineAbortHistoryDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <IncomingMachineAbortHistoryEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IncomingMachineAbortHistoryCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <IncomingMachineAbortHistoryFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IncomingMachineAbortHistorySeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IncomingMachineAbortHistoryPage);
