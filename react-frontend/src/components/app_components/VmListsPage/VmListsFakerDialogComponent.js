...
import vmListsFakerFactory from "./vmListsFakerFactory";
...
const VmListsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = vmListsFakerFactory(props.user,fakerCount,vmCodeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vmLists").create(elem));
...
export default connect(mapState, mapDispatch)(VmListsFakerDialogComponent);
