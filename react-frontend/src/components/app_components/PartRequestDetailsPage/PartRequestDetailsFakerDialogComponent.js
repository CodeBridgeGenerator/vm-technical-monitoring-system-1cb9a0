...
import partRequestDetailsFakerFactory from "./partRequestDetailsFakerFactory";
...
client.service("jobStations")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setjobIdIds(data.data);
                        });
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = partRequestDetailsFakerFactory(props.user,fakerCount,partNameIds,jobIdIds,TechnicianIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("partRequestDetails").create(elem));
...
export default connect(mapState, mapDispatch)(PartRequestDetailsFakerDialogComponent);
