...
import customerSalesOrdersFakerFactory from "./customerSalesOrdersFakerFactory";
...
const CustomerSalesOrdersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = customerSalesOrdersFakerFactory(props.user,fakerCount,companyIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customerSalesOrders").create(elem));
...
export default connect(mapState, mapDispatch)(CustomerSalesOrdersFakerDialogComponent);
