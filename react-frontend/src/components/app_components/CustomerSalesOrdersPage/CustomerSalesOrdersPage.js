...
import CustomerSalesOrdersDatatable from "./CustomerSalesOrdersDataTable";
import CustomerSalesOrdersEditDialogComponent from "./CustomerSalesOrdersEditDialogComponent";
import CustomerSalesOrdersCreateDialogComponent from "./CustomerSalesOrdersCreateDialogComponent";
import CustomerSalesOrdersFakerDialogComponent from "./CustomerSalesOrdersFakerDialogComponent";
import CustomerSalesOrdersSeederDialogComponent from "./CustomerSalesOrdersSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const CustomerSalesOrdersPage = (props) => {
    ...
    const filename = "customerSalesOrders.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("customerSalesOrders");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , company : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Sales Orders", type: "error", message: error.message || "Failed get Sales Orders" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("customerSalesOrders").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Sales Orders", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("customerSalesOrders").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/customerSalesOrders/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="customerSalesOrders-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="customerSalesOrders-datatable">
                    <CustomerSalesOrdersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CustomerSalesOrdersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CustomerSalesOrdersCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CustomerSalesOrdersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CustomerSalesOrdersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CustomerSalesOrdersPage);
