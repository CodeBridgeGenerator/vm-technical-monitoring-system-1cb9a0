...
import LocationMasterDatatable from "./LocationMasterDataTable";
import LocationMasterEditDialogComponent from "./LocationMasterEditDialogComponent";
import LocationMasterCreateDialogComponent from "./LocationMasterCreateDialogComponent";
import LocationMasterFakerDialogComponent from "./LocationMasterFakerDialogComponent";
import LocationMasterSeederDialogComponent from "./LocationMasterSeederDialogComponent";
...
const LocationMasterPage = (props) => {
    ...
    const filename = "locationMaster.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("locationMaster");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Location Master", type: "error", message: error.message || "Failed get Location Master" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("locationMaster").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Location Master", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("locationMaster").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/locationMaster/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="locationMaster-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="locationMaster-datatable">
                    <LocationMasterDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <LocationMasterEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <LocationMasterCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <LocationMasterFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <LocationMasterSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(LocationMasterPage);
