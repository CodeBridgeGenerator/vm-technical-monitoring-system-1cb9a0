...
import locationMasterFakerFactory from "./locationMasterFakerFactory";
...
const LocationMasterFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = locationMasterFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("locationMaster").create(elem));
...
export default connect(mapState, mapDispatch)(LocationMasterFakerDialogComponent);
