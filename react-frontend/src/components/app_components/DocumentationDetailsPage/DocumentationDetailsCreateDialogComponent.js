...
const DocumentationDetailsCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [documentationFile], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.fileName)) {
                error["fileName"] = `File Name field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            fileName: _entity?.fileName,documentationFile: _entity?.documentationFile?._id,
        ...
        const result = await client.service("documentationDetails").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info DocumentationDetails updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in DocumentationDetails" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create DocumentationDetails" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="documentationDetails-create-dialog-component">
