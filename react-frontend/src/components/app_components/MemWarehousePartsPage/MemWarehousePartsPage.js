...
import MemWarehousePartsDatatable from "./MemWarehousePartsDataTable";
...
import MemWarehousePartsFakerDialogComponent from "./MemWarehousePartsFakerDialogComponent";
import MemWarehousePartsSeederDialogComponent from "./MemWarehousePartsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MemWarehousePartsPage = (props) => {
    ...
    const filename = "memWarehouseParts.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("memWarehouseParts");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , part : urlParams.singleMemPartsId ,warehouse : urlParams.singleWarehouseMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Mem Warehouse Parts", type: "error", message: error.message || "Failed get Mem Warehouse Parts" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("memWarehouseParts").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Mem Warehouse Parts", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("memWarehouseParts").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Mem Warehouse Parts",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/memWarehouseParts/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Mem Warehouse Parts </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="memWarehouseParts-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="memWarehouseParts-datatable">
                    <MemWarehousePartsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MemWarehousePartsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MemWarehousePartsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MemWarehousePartsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MemWarehousePartsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MemWarehousePartsPage);
