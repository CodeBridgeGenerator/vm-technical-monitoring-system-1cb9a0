...
const MemWarehousePartsEditDialogComponent = (props) => {
...
reorderingPoint: _entity?.reorderingPoint,
        ...
        await client.service("memWarehouseParts").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info memWarehouseParts updated successfully" });
...
const warehouseOptions = warehouse.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Mem Warehouse Parts" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="memWarehouseParts-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(MemWarehousePartsEditDialogComponent);
