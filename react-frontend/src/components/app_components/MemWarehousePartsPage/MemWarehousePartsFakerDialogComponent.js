...
import memWarehousePartsFakerFactory from "./memWarehousePartsFakerFactory";
...
const MemWarehousePartsFakerDialogComponent = (props) => {
...
const [warehouseIds, setwarehouseIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("memParts")
                        .find({ query: { $select: ["_id","item"] } })
                        .then((data) => {
...
client.service("warehouseMaster")
    ...
    const onRun = async () => {
        let fakeData = memWarehousePartsFakerFactory(props.user,fakerCount,partIds,warehouseIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memWarehouseParts").create(elem));
...
export default connect(mapState, mapDispatch)(MemWarehousePartsFakerDialogComponent);
