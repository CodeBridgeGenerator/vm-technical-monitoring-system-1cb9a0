...
import SalesOrderItemsDatatable from "./SalesOrderItemsDataTable";
...
import SalesOrderItemsFakerDialogComponent from "./SalesOrderItemsFakerDialogComponent";
import SalesOrderItemsSeederDialogComponent from "./SalesOrderItemsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const SalesOrderItemsPage = (props) => {
    ...
    const filename = "salesOrderItems.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("salesOrderItems");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("salesOrderItems")
            .find({ query: { $limit: 10000 , salesOrder : urlParams.singleCustomerSalesOrdersId ,part : urlParams.singlePartsMasterId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Sales Order Items", type: "error", message: error.message || "Failed get Sales Order Items" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("salesOrderItems").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Sales Order Items", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("salesOrderItems").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/salesOrderItems/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="salesOrderItems-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="salesOrderItems-datatable">
                    <SalesOrderItemsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <SalesOrderItemsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SalesOrderItemsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <SalesOrderItemsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SalesOrderItemsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(SalesOrderItemsPage);
