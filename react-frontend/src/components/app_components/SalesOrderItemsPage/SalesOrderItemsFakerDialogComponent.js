...
import salesOrderItemsFakerFactory from "./salesOrderItemsFakerFactory";
...
const SalesOrderItemsFakerDialogComponent = (props) => {
...
const [partIds, setpartIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("customerSalesOrders")
                        .find({ query: { $select: ["_id","salesOrderId"] } })
                        .then((data) => {
                        setsalesOrderIds(data.data);
                        });
client.service("partsMaster")
    ...
    const onRun = async () => {
        let fakeData = salesOrderItemsFakerFactory(props.user,fakerCount,salesOrderIds,partIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("salesOrderItems").create(elem));
...
export default connect(mapState, mapDispatch)(SalesOrderItemsFakerDialogComponent);
