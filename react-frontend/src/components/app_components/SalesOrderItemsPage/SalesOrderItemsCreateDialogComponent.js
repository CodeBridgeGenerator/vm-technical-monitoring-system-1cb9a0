...
const SalesOrderItemsCreateDialogComponent = (props) => {
...
const [part, setPart] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [salesOrder,part], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            salesOrder: _entity?.salesOrder?._id,part: _entity?.part?._id,quantity: _entity?.quantity,
        ...
        const result = await client.service("salesOrderItems").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Sales Order Items updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Sales Order Items" });
        }
        setLoading(false);
    };
...
const partOptions = part.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Sales Order Items" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="salesOrderItems-create-dialog-component">
...
export default connect(mapState, mapDispatch)(SalesOrderItemsCreateDialogComponent);
